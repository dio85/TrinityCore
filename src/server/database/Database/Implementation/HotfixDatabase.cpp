...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    // Campaign.db2
    PrepareStatement(HOTFIX_SEL_CAMPAIGN, "SELECT ID, Title, Description, UiTextureKitID, RewardQuestID, Prerequisite, "
    ...
    PrepareStatement(HOTFIX_SEL_CHR_CUSTOMIZATION_OPTION, "SELECT Name, ID, SecondaryID, Flags, ChrModelID, SortIndex, "
    ...
    PrepareStatement(HOTFIX_SEL_DIFFICULTY, "SELECT ID, Name, InstanceType, OrderIndex, OldEnumValue, FallbackDifficultyID, MinPlayers, MaxPlayers, "
        "Flags, ItemContext, ToggleDifficultyID, GroupSizeHealthCurveID, GroupSizeDmgCurveID, GroupSizeSpellPointsCurveID, Field_11_0_5_56487_013 FROM difficulty"
        " WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(HOTFIX_SEL_GROUP_FINDER_CATEGORY, "SELECT ID, Name, Description, OrderIndex, Flags FROM group_finder_category"
        " WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    // QuestPoiBlob.db2
    PrepareStatement(HOTFIX_SEL_QUEST_POI_BLOB, "SELECT ID, MapID, UiMapID, NumPoints, QuestID, ObjectiveIndex, ObjectiveID, PlayerConditionID, NavigationPlayerConditionID FROM quest_poi_blob WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    // ZoneStory.db2
    PrepareStatement(HOTFIX_SEL_ZONE_STORY, "SELECT ID, PlayerFactionGroupID, DisplayAchievementID, DisplayUIMapID, PlayerUIMapID FROM zone_story WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
